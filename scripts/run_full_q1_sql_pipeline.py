...
def main():
    ...
    run_command("docker exec spark-master spark-submit /opt/spark/work-dir/sql/q1-sql.py")

    print("📦 Recupero ultimo path di output HDFS...")
    hdfs_list_output = run_command(
        "docker exec namenode hdfs dfs -ls /output | grep q1_sql_ | awk '{print $8}' | sort | tail -n 1",
        capture_output=True
    )

    last_output_path = hdfs_list_output.strip()
    if not last_output_path:
        print("❌ Nessun path trovato in /output per q1_sql_")
        sys.exit(1)
    ...
    run_command(
        f"docker exec namenode bash -c \"hdfs dfs -getmerge {last_output_path} /shared_data/q1_sql_result.csv && "
        f"echo '✅ File CSV pronto in /shared_data/q1_sql_result.csv'\""
    )

    print("📁 File CSV disponibile nella directory montata: ./hdfs/shared_data/q1_sql_result.csv")
